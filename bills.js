...
function load() {
    ...
    for (let i = 0; i < draftOrder.length; i++) {
      for (let j = 0; j < draftOrder[i].length; j++) {
        place += 1;
        if (draftOrder[i][j].name === buf.name) {
          console.log('#' + place);
        }
      }
    }
...
function finishTrade(type) {
  if (type === "watson") {
    ...
    for (var i = 0; i < darnoldTradeArr[darnoldNav].receiveIndex.length; i++) {
      draftOrder[darnoldTradeArr[darnoldNav].receiveIndex[i][0]][darnoldTradeArr[darnoldNav].receiveIndex[i][1]] = buf;
    ...
    for (var i = 0; i < twoTradeArr[twoNav].receiveIndex.length; i++) {
      draftOrder[twoTradeArr[twoNav].receiveIndex[i][0]][twoTradeArr[twoNav].receiveIndex[i][1]] = buf;
...
////
    ...
    console.log(getCapRoom() - ((teamFA[i].cTotal * 1000000)/(teamFA[i].cYears)));

    if (!offeredArr.includes(teamFA[i]) && !(getCapRoom() - ((teamFA[i].cTotal * 1000000)/(teamFA[i].cYears)) < 0)) {
      ...
      signButton.style.backgroundColor = "#00338e"
    ...
    } else if (getCapRoom() - ((teamFA[i].cTotal * 1000000)/(teamFA[i].cYears)) < 0) {
      var notEnoughCol = document.createElement("div");
      notEnoughCol.classList.add("col-12");

      var notEnoughP = document.createElement("p");
      notEnoughP.innerHTML = "NOT ENOUGH SPACE";

      notEnoughCol.appendChild(notEnoughP);
      row.appendChild(notEnoughCol);
    }
    // if () {
...
function generateRoster() {
  ...
  row.style.borderBottom = "1px solid #00338e";
  ...
  for (let i = 0; i < activeRoster.length; i++) {
    ...
    if (!signedArr.includes(activeRoster[i]) && cutArr.length < 6) {
      ...
      if (num < 0) {
        penP.style.fontWeight = "bold";
        penP.style.color = "white";
      }
 ...
 ///<i class="fa fa-minus-circle" aria-hidden="true"></i>
      ...
      icon.classList.add("fa", "fa-times-circle", "fa-lg");
...
function generateYourPickHistory() {
  ...
  for (let i = (billsDrafted.length - 1); i >= 0; i--) {
    var row = document.createElement("div");
    row.classList.add("row");
    ...
    teamP.setAttribute("src", buf.logo);
    ...
    pickNum = billsDrafted[i].pickNum;
    ...
    nameP.innerHTML = billsDrafted[i].name;
    nameCol.appendChild(nameP);
    row.appendChild(nameCol);
    ...
    schoolP.innerHTML = billsDrafted[i].school;
...
function draftsOver() {
  ...
  // console.log("\nDRAFTED\n")
  for (var i = 0; i < billsDrafted.length; i++) {
    // console.log(billsDrafted[i].name);
  }
}
...
function generateSummary() {
  ...
  if (billsDrafted.length != 0) {
    for (var i = 0; i < draftSummary.length; i++) {
      if (draftSummary[i][0].name === "Bills") {
        var row = document.createElement("div");
        row.classList.add("row");
        var col = document.createElement("div");
        col.classList.add("col-12");
  ...
  for (var i = n[0]; i < draftOrder.length + 1; i++) {
    ...
    for (var k = n[1]; k < draftOrder[i].length; k++) {
      ...
      if (draftOrder[i][k] != buf) {
        ...
        console.log("We hit the else (please no)!");
        console.log("i = " + i);
...
function pickTradeScreen() {
  ...
  if (pickTradeOn === true) {
    ...
    tradeButton.style.backgroundColor = "#3b84ec";
...
function generateTeamPicks(team) {
  ...
  for (let k = 0; k < buf.picks.length; k++) {
    var p = document.createElement("p");
    p.classList.add("teamPick");
    var round = buf.picks[k][0] + 1;
    var pick = buf.picks[k][1] + 1;
    p.innerHTML =  "R" + round + " P" + pick;
    p.setAttribute("id", "r" + buf.picks[k][0] + "p" + buf.picks[k][1])
...
function addSend(pick) {
  ...
  if (p.classList.contains("checked")) {
    ...
    console.log("not ok");
    p.style.backgroundColor = "#3b84ec";
...
function pickOffer() {
  ...
  if (result) {
    ...
    for (var i = 0; i < picksGet.length; i++) {
      draftOrder[picksGet[i][0]][picksGet[i][1]] = buf;
